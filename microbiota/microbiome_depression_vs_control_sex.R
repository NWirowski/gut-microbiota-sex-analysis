...
# Ensure we only keep Depression and control groups in each subset
ps_female <- subset_samples(ps_female, Disorder %in% c("Control", "Depression"))
ps_male <- subset_samples(ps_male, Disorder %in% c("Control", "Depression"))
...
# Salvar os dados
write.table(alpha_diversity_female, file = "C:/Users/natal/Documents/data/scripts/results/alpha_diversity_female_DXC.txt", sep = "\t", row.names = TRUE, quote = FALSE)
write.table(alpha_diversity_male, file = "C:/Users/natal/Documents/data/scripts/results/alpha_diversity_male_DXC.txt", sep = "\t", row.names = TRUE, quote = FALSE)
...
g_bray_female <- plot_pcoa(pcoa_bray_female, ps_female, "PCoA - Bray-Curtis (Female - Depression vs Control)")
g_bray_male   <- plot_pcoa(pcoa_bray_male, ps_male, "PCoA - Bray-Curtis (Male - Depression vs Control)")
...
g_nmds_bray_female <- plot_nmds(nmds_bray_female, ps_female, "NMDS - Bray-Curtis (Female - Depression vs Control)")
g_nmds_bray_male   <- plot_nmds(nmds_bray_male,   ps_male,   "NMDS - Bray-Curtis (Male - Depression vs Control)")
...
write.table(permanova_results,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\PERMANOVA_sexDXC.txt",
...
write.table(anosim_results_all,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\ANOSIM_sexDXC.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
...
sample_data(ps_female)$Disorder <- relevel(as.factor(sample_data(ps_female)$Disorder), ref = "Control")
...
write.table(res_female_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_female_DXC_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_female_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_female_DXC_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)

# Male Differential Abundance
sample_data(ps_male)$Disorder <- as.factor(sample_data(ps_male)$Disorder)
sample_data(ps_male)$Disorder <- relevel(as.factor(sample_data(ps_male)$Disorder), ref = "Control")
...
# Export as tab-separated .txt files
write.table(res_male_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_male_DXC_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_male_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_male_DXC_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
# o primeiro nível listado é "Control", isso significa que o grupo de 
# referência no DESeq2 é "Control".
# Se o log2FoldChange é positivo, isso indica que a ASV é menos abundante no grupo
# de referência ("Control") e mais abundante no grupo "Depression".
# Se o log2FoldChange é negativo, isso indica que a ASV é mais abundante no grupo
# de referência ("Control") e menos abundante no grupo "Depression".
...
volcano_female_plot <- ggplot(res_female_df, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  labs(
    title = "Volcano Plot - Female - Depression vs Control",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
...
volcano_male_plot <- ggplot(res_male_df, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black") +  # Linha de corte para padj < 0.05
  labs(
    title = "Volcano Plot - Male - Depression and Control",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
