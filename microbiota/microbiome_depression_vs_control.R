...
# Adjust dataframe to analyze disorder vs. control: depression
df_samples <- df_samples %>% 
    filter(mood %in% c(0,3)) %>% 
    mutate(group = if_else(mood != 0, "Depression", "Control"))
...
writeLines(
  ...
  con = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\proporcao_mantida_DXC.txt"
)
...
rownames(track) <- df_samples$id
track_df <- as.data.frame(track)

write.table(track_df,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\track_DXC.txt",
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)


#### Conferindo se não tem os primer ####
# Convert to a data frame and transpose it
# asv_table <- as.data.frame(t(seqtab.nochim))

# Add the ASV sequences as a column (they are currently rownames)
# asv_table$sequence <- rownames(asv_table)

# Optional: move sequence column to the first column
# asv_table <- asv_table[, c(ncol(asv_table), 1:(ncol(asv_table)-1))]

# Or as TSV (tab-separated)
# write.table(asv_table, file = "asv_sequences_table_DXC.tsv", sep = "\t", quote = FALSE, row.names = FALSE)

### Só a sequência
# Extract just the sequences
# sequences <- asv_table$sequence

# Write to a simple text file (one sequence per line)
# writeLines(sequences, "asv_sequences_DXC.txt")
...
saveRDS(taxa, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa_DXC.rds")

# Create Phyloseq object
samdf <- df_samples %>%
  select(id, group, sexo, gravidade, gravidade_cat, bdi_dico, bdi_sem_leve_ms) %>%
  rename(Subject = id, Disorder = group, Sex = sexo, Severity = gravidade, Severity_cat = gravidade_cat, BDI_dico = bdi_dico, BDI_cat = bdi_sem_leve_ms) %>%
...
saveRDS(ps, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\phyloseq_object_DXC.rds")
...
write.table(alpha_diversity, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\alpha_diversity_DXC.txt", sep = "\t", row.names = TRUE, quote = FALSE)
...
write.table(alpha_pvalues, 
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\alpha_diversity_pvalues_DXC.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE)

# Boxplot of Shannon diversity by group
alpha_diversity$Disorder <- factor(alpha_diversity$Disorder, levels = c("Control", "Depression"))
...
plot_alpha_diversity <- function(data, metric, pval) {
  ...
  ggplot(data, aes(x = Disorder, y = .data[[metric]], fill = Disorder)) +
    ...
    scale_fill_manual(values = c("Control" = "#79a2cd", "Depression" = "#e57a74"))
}
...
plot_pcoa <- function(ordination, title) {
  ...
  plot_ordination(ps, ordination, color = "Disorder") +
    ...
    theme_minimal() +
    scale_color_manual(values = c("Control" = "#79a2cd", "Depression" = "#e57a74"))
}
...
write.table(df_bray, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\PERMANOVA_DXC.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(anosim_results,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\ANOSIM_DXC.txt",
...
# Set 'control' as the reference level for 'Depression'
...
write.table(res_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_DXC_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_DXC_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
volcano_plot <- ggplot(res_df_clean, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  labs(
    title = "Volcano Plot - Depression vs Control",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
...
#### SAVE OBJECTS ####
# saveRDS(df_samples, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\df_samples_DXC.rds")
# saveRDS(filtered_sequences, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\filtered_sequences_DXC.rds")
# saveRDS(errF, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\errF_DXC.rds")
# saveRDS(dada_reads, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\dada_reads_DXC.rds")
# saveRDS(seqtab.nochim, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\seqtab.nochim_DXC.rds")
# saveRDS(taxa, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa_DXC.rds")
# saveRDS(taxa.print, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa.print_DXC.rds")
# saveRDS(samdf, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\samdf_DXC.rds")
# saveRDS(ps, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\phyloseq_object_DXC.rds")
# saveRDS(ps, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\phyloseq_object_tree_DXC.rds")
# saveRDS(dna, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\dna_DXC.rds")
# saveRDS(fastqs_filtered, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\fastqs_filtered_DXC.rds")
# saveRDS(sample.names, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\sample.names_DXC.rds")
# saveRDS(samples.out, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\samples.out_DXC.rds")
