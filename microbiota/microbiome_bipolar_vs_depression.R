...
# Adjust dataframe to analyze disorder vs. depression
df_samples <- df_samples %>% 
    filter(mood %in% c(2,3)) %>% 
    mutate(group = if_else(mood == 2, "Bipolar", "Depression"))
...
writeLines(
  ...
  con = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\proporcao_mantida_BXD.txt"
)
...
write.table(track_df,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\track_BXD.txt",
...
taxa
saveRDS(taxa, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa_BXD.rds")
...
length(rownames(seqtab.nochim))  # deve ser 55
head(rownames(samdf))
length(rownames(samdf))  # deve ser 55
setdiff(rownames(seqtab.nochim), rownames(samdf))  # O que tem em seqtab e não tem em samdf
setdiff(rownames(samdf), rownames(seqtab.nochim))  # O que tem em samdf e não tem em seqtab
...
write.table(alpha_diversity, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\alpha_diversity_BXD.txt", sep = "\t", row.names = TRUE, quote = FALSE)
...
write.table(shapiro_results,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\shapiro_alpha_diversity_BXD.txt",
            sep = "\t", 
            row.names = FALSE,
            quote = FALSE)
...
write.table(alpha_pvalues, 
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\alpha_diversity_pvalues_BXD.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE)

# Boxplot of Shannon diversity by group
alpha_diversity$Disorder <- factor(alpha_diversity$Disorder, levels = c("Depression", "Bipolar"))
...
write.table(df_bray, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\PERMANOVA_BXD.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(anosim_results,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\ANOSIM_BXD.txt",
...
# Set 'Depression' as the reference level for 'Bipolar'
sample_data(ps)$Disorder <- as.factor(sample_data(ps)$Disorder)
sample_data(ps)$Disorder <- relevel(sample_data(ps)$Disorder, ref = "Depression")
...
write.table(res_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_BXD_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_BXD_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
volcano_plot <- ggplot(res_df_clean, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  labs(
    title = "Volcano Plot - Bipolar vs Depression",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
...
# saveRDS(seqtab.nochim, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\seqtab.nochim_BXD.rds")
# saveRDS(taxa, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa_BXD.rds")
# saveRDS(taxa.print, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\taxa.print_BXD.rds")
# saveRDS(samdf, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\samdf_BXD.rds")
# saveRDS(ps, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\phyloseq_object_BXD.rds")
# saveRDS(ps, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\phyloseq_object_tree_BXD.rds")
# saveRDS(dna, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\dna_BXD.rds")
# saveRDS(fastqs_filtered, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\fastqs_filtered_BXD.rds")
# saveRDS(sample.names, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\sample.names_BXD.rds")
# saveRDS(samples.out, file = "C:\\Users\\natal\\Documents\\data\\scripts\\objects\\samples.out_BXD.rds")
