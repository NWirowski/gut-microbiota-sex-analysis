...
# Ensure we only keep bipolar and control groups in each subset
ps_female <- subset_samples(ps_female, Disorder %in% c("Control", "Bipolar"))
ps_male <- subset_samples(ps_male, Disorder %in% c("Control", "Bipolar"))
...
# Salvar os dados
write.table(alpha_diversity_female, file = "C:/Users/natal/Documents/data/scripts/results/alpha_diversity_female_BXC.txt", sep = "\t", row.names = TRUE, quote = FALSE)
write.table(alpha_diversity_male, file = "C:/Users/natal/Documents/data/scripts/results/alpha_diversity_male_BXC.txt", sep = "\t", row.names = TRUE, quote = FALSE)
...
g_bray_female <- plot_pcoa(pcoa_bray_female, ps_female, "PCoA - Bray-Curtis (Female - Bipolar vs Control)")
g_bray_male   <- plot_pcoa(pcoa_bray_male, ps_male, "PCoA - Bray-Curtis (Male - Bipolar vs Control)")
...
g_nmds_bray_female <- plot_nmds(nmds_bray_female, ps_female, "NMDS - Bray-Curtis (Female - Bipolar vs Control)")
g_nmds_bray_male   <- plot_nmds(nmds_bray_male,   ps_male,   "NMDS - Bray-Curtis (Male - Bipolar vs Control)")
...
write.table(permanova_results,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\PERMANOVA_sexBXC.txt",
...
write.table(anosim_results_all,
            file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\ANOSIM_sexBXC.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
...
write.table(res_female_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_female_BXC_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_female_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_female_BXC_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
# Export as tab-separated .txt files
write.table(res_male_up, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_male_BXC_up.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(res_male_down, file = "C:\\Users\\natal\\Documents\\data\\scripts\\results\\deseq_male_BXC_down.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
# de referência ("Control") e mais abundante no grupo "Bipolar".
# Se o log2FoldChange é negativo, isso indica que a ASV é mais abundante no grupo
# de referência ("Control") e menos abundante no grupo "Bipolar".
...
volcano_female_plot <- ggplot(res_female_df, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  labs(
    title = "Volcano Plot - Female - Bipolar vs Control",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
...
volcano_male_plot <- ggplot(res_male_df, aes(x = log2FoldChange, y = `-log10(padj)`, color = Significance)) +
  ...
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black") +  # Linha de corte para padj < 0.05
  labs(
    title = "Volcano Plot - Male - Bipolar and Control",
    x = "Log2 Fold Change",
    y = "-log10(padj)"
  ) +
  theme_minimal()
